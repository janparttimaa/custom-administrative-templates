...
> [!IMPORTANT]  
> Unfortunately you cannot set "enabled_social_providers" using this ADMX-template. Instead you need to use another methods. Some of these methods are listed below:
...
| ```bDropboxConnectorEnabled``` | [See below](#bDropboxConnectorEnabled)  | ```./Device/Vendor/MSFT/Policy/Config/AdobeAcrobatDC~Policy~Cat_Adobe_Acrobat_DC~Cat_Preferences~Cat_General/bDropboxConnectorEnabled``` | String | ```<enabled/>```<br>```<disabled/>```
