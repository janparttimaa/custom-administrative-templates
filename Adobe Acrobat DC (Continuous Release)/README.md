...
> [!NOTE]  
> These ADMX and ADML templates are forked and further developed from [original templates](https://www.adobe.com/devnet-docs/acrobatetk/tools/DesktopDeployment/gpo.html) provided by Adobe as the Adobe's original templates covers only few settings, are not actively updated and developed by Adobe plus the Adobe's original templates are not suitable for Intune-deployment using ingestion.
